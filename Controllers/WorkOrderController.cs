...
public class WorkOrderController : ControllerBase
{
    ...
    [HttpPut("close/{id}")]
    [Authorize]
    public IActionResult CloseWorkOrder(WorkOrder workOrder, int id)
    {
        WorkOrder workOrderToClose = _dbContext.WorkOrders.SingleOrDefault(wo => wo.Id == id);

        if (workOrderToClose == null)
        {
            return NotFound();
        }
        else if (id != workOrder.Id)
        ...
        workOrderToClose.DateCompleted = DateTime.Now;
        _dbContext.SaveChanges();
    ...
    [HttpDelete("{id}")]
    [Authorize]
    public IActionResult DeleteIncompleteWorkOrder(WorkOrder workOrder, int id)
    {
